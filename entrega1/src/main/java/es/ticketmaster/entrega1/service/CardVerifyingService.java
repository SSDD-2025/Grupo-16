...
public class CardVerifyingService {
    ...
    /**
     * This method will verify if the cardHolder is a valid name. It will be a valid name if:
     *  - The name begin with letters, could contain blank spaces and the length of the name must be between 2 and 50 characters. 
     *    This is spciefied by the regular expression shown below.
     *  - The name cannot be a blank space.
     *  - Can not be null neither, however, a NullPointerException will be thrown automatically by the compiler.
    ...
    /**
     * In order to verify the card number, the procedure to follow is applying the Luhn`s Algorithm, 
     * which help us check the card`s validity.
     * Note: This algorithm was implemented without any modifications, comparing it to the original.
     * @param number is the credit card number.
     * @return true if it is a valid number (if the accumulate sum is divisible by 10), false if not.
     */
    public boolean verifyCreditCardNumber(String number) {
        number = number.replaceAll("\\s", ""); /* All the blank spaces are removed, in order to verify the number itself. */
        ...
        if (!number.matches("\\d{15,16}")) { 
            ...
            int accumulateSum = 0; /* Is the accumulate sum of all the digits in the card number,
            ...
            boolean alternate = false; /* Used to toggle between multiplying by 2 and not multiplying. 
                                       This is done because, in Luhn's algorithm, every second digit (starting from the end) 
                                       is multiplied by 2. */
            for (int i = number.length() - 1; i >= 0; i--) {
                int digit = Character.getNumericValue(number.charAt(i)); /* Obtain the digit at a position i of the card number. */
                if (alternate) {
    ...
    /**
     * This method will verify if the card number introduced, matches with any of the 3 types of cards that the user can choose
     * (MasterCard, Visa or American Express).
     * @param number is the card number.
     * @return If the number match, it will return the type name of the user´s card. If not, null.
     */
    ...
    /**
     * This method will verify if the expiration date introduced by the user is realistic, 
     * that is, it is not a previous date compared to the date on which the form is being made.
     * @param date is the expiration date introduced by the user.
     * @return true if the expiration date is realistic, false (along with a DateTimeParseException) if not.
     */
    public boolean verifyExpirationDate(String date) {
        try {
            DateTimeFormatter dateFormat = DateTimeFormatter.ofPattern("yyyy-MM"); /* Stablish the format that the date must have. */
            YearMonth expirationDateFormatted = YearMonth.parse(date, dateFormat); /* Transform the date into the correct format. 
            If it cannot be transform, a DateTimeParseException will be captured by the catch block. */
            return expirationDateFormatted.isAfter(YearMonth.now()); /* If the expiration date is later than 
    ...
    /**
     * This method will verify if the card´s security number (CVV) is a valid one.
    ...
    public boolean verifyCVV(String cvv) {
        ...
        if ((cvv.length() == NORMAL_CVV_LENGTH) || (cvv.length() == AMERICAN_EXPRESS_CVV_LENGTH)) {
            int i = 0;
            while (i < cvv.length()) {
                if (!Character.isDigit(cvv.charAt(i))) { /* If one of its number is not an integer (digit), it will return false and exit the loop. */
                    return false;
                }
                else { /* Otherwise, it will continue verifying the cvv. */
                    i++;
                }
            }
            return true; /* If the loop checked all the cvv numbers, then the cvv is a valid one. */
        }
        else {
            return false;
        }
